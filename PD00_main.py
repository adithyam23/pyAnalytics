...
#%%describing
mtcarsDF.shape #rows and columns
mtcarsDF.head(3) #First 3 data
mtcarsDF.tail(4) #Last 4 data
mtcarsDF.describe() #Shows stastics - mean,quartiles etc
mtcarsDF.columns #Column names
mtcarsDF.dtypes #Data types of the columns

mtcarsDF.index  #here index by rownames
type(mtcarsDF) #Tells type

mtcarsDF.select_dtypes(include=['int64']) #Columns that have int
mtcarsDF.select_dtypes(exclude=['int64']) #Columns not having int
mtcarsDF.isna() #Returns True if value is missing
mtcarsDF.notna() #Returns True if data is not missing
id(mtcarsDF) #Dataframe identity
mtcars.empty #Indicate whether dataframe empty row or column
mtcars.size #Size in bytes
mtcars.ndim #
mtcars.axes #Tells both row and column name
mtcars.values
